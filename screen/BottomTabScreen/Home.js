...
import { getProposition, unDoSwipe, swipeHandler} from "../../BackEnd/controllers/cards";
  ...
  return (
    <View style={styles.buttonContainer}>
      <PrimaryButton
        ...
        color="red"
      />
      <TouchableOpacity onPress={() => swiperRef.current.swipeBottom()} style={{ height: 55, width: 55, justifyContent: "center", alignItems: "center" }}>
...
export default function Home({navigation, route}) {
  ...
  async function handleUndoPress() {
    ...
    if (absoluteIndex===0) {
      Alert.alert(
        "Impossible !",
        "Tu te trouves déjà sur la première carte",
        [{ text: "Ok", style: "cancel"},
        ]
      ); 
      ...
      if (listIndex===0) {
        ...
        setListIndex((i) => i-1);
        const undoSuccess = await unDoSwipe();
        (!!undoSuccess && !undoSuccess?.error) ? null : (() => {
          // si erreur :
  ...
  useLayoutEffect(() => {
    navigation.setOptions({
      headerRight: () => (
        <HeaderButton onSettingsPress={onSettingsPress} onUndoPress={onUndoPress} onManageSwipeButtonPressed={onManageSwipeButtonPressed}/>
      ),
    });
  }, [navigation]);
  ...
  if (isPropositionLoaded) {
    return (
      <View style={styles.mainContainer}>
        ...
        <View style={[styles.bottomContainer, {zIndex: swipeButtonZIndex}]}>{swipeButtonValue}</View>
