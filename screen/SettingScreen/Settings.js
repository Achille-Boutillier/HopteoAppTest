import { View, StyleSheet, Text, TouchableOpacity, TextInput, Linking } from "react-native";
...
import { deleteUser, disconnect, getUserInfo, getAppContact, getAppInfo } from "../../BackEnd/controllers/setting";
...
export default function Settings({navigation}) {
  ...
  async function handleAppInfo() {
    ...
    appInfoNavigation(appInfo);
      ...
      if (!!data && !data.error) {
        ...
        appInfoNavigation({errorMessage : "Une erreur est survenue dans le chargement des données..." });
      }
    }
  }
  ...
  function onPressReset() {
    setModalBodyText("Tu ne pourras plus revenir en arrière une fois ton compte réinitialisé ! Ton classement sera perdu...");
  ...
  function onPressDeleteAccount() {
    setModalBodyText("Tu ne pourras plus revenir en arrière un fois ton compte supprimé...");
  ...
  return (
    <View style={styles.mainContainer}>
      ...
      <View style={styles.bodyContainer}>
        ...
        <SettingsSection iconName={"information-circle-outline"} optionTitle={"A propos de l'application"} onPress={onPressAppInfo} />
        <SettingsSection iconName={"chatbubble-outline"} optionTitle={"Nous contacter"} onPress={handleContactUsmodal} />
        <SettingsSection iconName={"lock-closed-outline"} optionTitle={"Modifier mon mot de passe"} onPress={onPressModifyPassword} />
        <SettingsSection iconName={"refresh"} optionTitle={"Reset tous mes swipes"} onPress={onPressReset} />
        <SettingsSection iconName={"trash-bin-outline"} optionTitle={"Supprimer mon compte"} onPress={onPressDeleteAccount} />
        <SettingsSection iconName={"log-out-outline"} optionTitle={"Se déconnecter"} onPress={onPressDisconnect} />
      ...
      <Modal isVisible={isContactUsModalVisible}>
        <View style={styles.modal}>
          ...
          <View style={styles.modalBody}>
            <Text style={styles.modalBodyText}>Pour tout problème ou recommandation, contacte l'adresse e-mail suivante :</Text>
            <TouchableOpacity onPress={() => Linking.openURL(`mailto:${appMail}`)}>
              <Text style={{color: Colors.orange500, textDecorationLine:'underline', fontSize:18 }}>{appMail}</Text>
            </TouchableOpacity>
