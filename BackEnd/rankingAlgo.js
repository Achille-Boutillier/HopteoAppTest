...
export function generateRanking(swipe, cards, schools, minSwipeForRanking, themeDetail, swipeSettings) {
    try {
        if (swipe.answeredList.length>=minSwipeForRanking) {  // Si l'utilisateur a swipé un certain nb de proposition
            ...
            let schoolName = {};
            const schoolIdList = schools.map((school)=>{
            ...
            for (let schoolId of schoolIdList) {
                if (!Object.values(schoolGroupObj[schoolId]).includes(0)) {  // Si l'école est concernée par le nouveau classement
                    ...
                    for (let idCard of swipe.answeredList) {
                        let weight = 0;
                        if (cardObj[idCard] && cardObj[idCard].schoolList.includes(schoolId)) {  //todo : ajout une condition d'existance de la carte
...
function createSchoolGroupObj(schoolIdList, cardObj, swipeObj) {
    ...
    const answeredIdCardList = Object.keys(swipeObj);  //todo : le trie se faisait sur toutes les cartes mais doit se faire que sur les cartes swipés
    for (let idCard of answeredIdCardList) {
        ...
        if (idGroupe!=="nc") {  // si la proposition a un groupe
            ...
            cardSchoolList.forEach((schoolId) => {
                if (schoolIdList.includes(schoolId)) {  //todo : ajout de cette condition
                    if (groupLength[idGroupe] === 1 && swipeObj[idCard] === "dislike") { // Si le groupe est constitué d'une seule prop et que la proposition est disliké
                        schoolGroupObj[schoolId][idGroupe] = 0; // On supprime l'école
                    } else if (groupLength[idGroupe] > 1 && swipeObj[idCard] !== "dislike") {  // Si le groupe contient plusieurs propositions Et qu'une proposition n'est pas disliké
